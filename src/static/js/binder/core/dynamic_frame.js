...
class DynamicFrame extends Controller {
    ...
    loadFragment() {
        if (this.args.fragmentKey) {
            ...
            if (fragment) {
                ...
                let fragmentParts = Object.fromEntries(fragment.split(";").map(part => part.split("=")));

                if (fragmentParts[`${this.args.fragmentKey}-url`]) {
                    this.args.url = fragmentParts[`${this.args.fragmentKey}-url`];
                    delete fragmentParts[`${this.args.fragmentKey}-url`];
                }

                for (let [key, value] of Object.entries(fragmentParts)) {
                    key = key.replace(`${this.args.fragmentKey}-param-`, "");
    ...
    saveFragment() {
        if (this.args.fragmentKey) {
            ...
            fragment[`${this.args.fragmentKey}-url`] = this.args.url;

            for (const [key, value] of this.params()) {
                fragment[`${this.args.fragmentKey}-param-${key}`] = value;
            }

            const fragmentString = Object.entries(fragment).map(part => `${part[0]}=${part[1]}`).join(";");
